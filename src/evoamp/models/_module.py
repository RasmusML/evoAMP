...
class Encoder(nn.Module):
    def __init__(self, input_dim: int, embedding_dim: int, hidden_dim: int, latent_dim: int):
        ...
        self.embedding = nn.Embedding(input_dim, embedding_dim)
        self.gru1 = nn.GRU(embedding_dim, hidden_dim, bidirectional=(self.directions == 2), batch_first=True)
        self.gru2 = nn.GRU(
            self.directions * hidden_dim, hidden_dim, bidirectional=(self.directions == 2), batch_first=True
        )
...
class VAE(nn.Module):
    def __init__(
        self, input_dim: int, embedding_dim: int, latent_dim: int, hidden_dim: int, seq_start_embedding: torch.Tensor
    ):
        super().__init__()
